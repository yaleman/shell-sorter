...
def mock_camera_manager_for_api():
    ...
    with patch('shell_sorter.app.camera_manager') as mock_manager:
        ...
        mock_manager.get_selected_cameras.return_value = [mock_camera, mock_network_camera]
...
class TestCameraAPI:
    ...
    def test_start_selected_cameras(self, test_client: TestClient, mock_camera_manager_for_api):
    ...
    def test_set_camera_view_type_invalid(self, test_client: TestClient, mock_camera_manager_for_api):
        """Test setting invalid camera view type."""
        response = test_client.post(
            "/api/cameras/0/view-type",
            data={"view_type": "invalid"}
        )
    ...
    def test_set_camera_region_invalid(self, test_client: TestClient, mock_camera_manager_for_api):
...
class TestConfigurationAPI:
    ...
    def test_save_configuration(self, mock_get_monitor, mock_get_settings, mock_camera_manager, test_client: TestClient):
